...
def _copy_one_media(value, media_dir, many=False):
    ...
    # Do not copy an already-rendered relative media URL again.
...
def _copy_media(manifest, data, target):
    ...
    # Keep canonical gallery/music values synchronized without copying
    # already-rendered media URLs a second time.
    ...
    if "gallery" in data and isinstance(data["gallery"], list):
        data["gallery"] = [str(x).replace("\\", "/") for x in data["gallery"] if x]
    if "music" in data and data["music"]:
        data["music"] = str(data["music"]).replace("\\", "/")
