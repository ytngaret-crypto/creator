...
async def collect_photo(m,state):
    ...
    # Simpan foto di buffer khusus + data field. Buffer khusus mencegah
    # foto dianggap kosong jika struktur data field berubah/tertimpa.
...
async def done(m,state):
    ...
    if f.get("type")=="photos":
        ...
        # Normalisasi kembali ke field manifest sebelum melanjutkan.
        if photos:
            data[f["id"]]=photos
            await state.update_data(data=data, photos_buffer=photos)
