...
async def collect_text(m,state):
    ...
    # Text fields must be written into the manifest field ID before advancing.
    # Without this, the wizard visually accepts the user's text but `data`
    # remains empty, so the final manifest validation reports required fields
    # such as title/intro1 as missing.
    data=dict(d.get("data") or {})
    if typ == "text_list":
        values=list(data.get(f["id"]) or [])
