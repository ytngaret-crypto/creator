...
async def choose_template(c,state):
    ...
    # Re-read the template from disk at the exact moment it is selected.
    # This prevents an old field list from surviving after a template/manifest
    # is edited or replaced on the server.
    fresh=get_template(t["category"], t["id"])
    if not fresh:
        await c.answer("Manifest template tidak ditemukan.",show_alert=True);return
    fields=list(fresh.get("fields") or [])
    if not fields:
        await c.answer("Template tidak memiliki fields valid di manifest.json.",show_alert=True);return
    await state.update_data(template_id=fresh["id"],fields=fields,index=0,data={})
    await track_message(state,c.message.message_id)
