...
from db import *
from templates import list_templates,get_template,get_template_by_key,list_categories,category_display_name,validate_template_dir
from builder import build_all
from netlify import deploy_directory

bot=Bot(BOT_TOKEN);dp=Dispatcher(storage=MemoryStorage(), events_isolation=SimpleEventIsolation())
...
async def choose_template(c,state):
    if not await ensure_access(c):return
    d=await state.get_data();t=get_template_by_key(c.data.split(":",1)[1])
...
async def ask_next(m,state):
    ...
    if typ in PHOTO_TYPES:
        if typ == "photo":
            ...
            suffix="Kirim foto satu per satu, lalu ketik /selesai."
        if not required:
...
async def skip_current(m,state):
    ...
    typ=str(f.get("type","text")).lower().strip()
    data[f["id"]] = [] if typ in PHOTO_TYPES and typ != "photo" else ""
...
async def done(m,state):
    # /selesai hanya diperlukan untuk field foto jamak.
    await track_message(state,m.message_id)
    d=await state.get_data()
    fields=d.get("fields",[])
    i=d.get("index",0)
    ...
    if typ in PHOTO_TYPES and typ != "photo":
        ...
        if f.get("required") and not photos:
            ...
            return
        await state.update_data(index=i+1,photos_buffer=[])
        await ask_next(m,state)
    else:
...
async def finish_creation(m,state):
    ...
    if not category or not template_id:
        await m.answer("❌ Sesi template tidak lengkap. Silakan mulai lagi dengan /start.")
        await state.clear()
        return

    # Final validation uses the SAME manifest fields that drove the wizard.
    # This prevents false errors caused by hard-coded field names and catches
    # missing uploads before anything is written to the database.
    missing=[]
    for f in fields:
        ...
        value=data.get(fid)
        typ=str(f.get("type") or "text").lower().strip()
        if typ in PHOTO_TYPES:
            values=value if isinstance(value,list) else ([value] if value else [])
            values=[str(x).strip() for x in values if str(x).strip()]
            if not values:
                missing.append(str(f.get("label") or fid))
        else:
            if value is None or not str(value).strip():
                missing.append(str(f.get("label") or fid))

    if missing:
        print(f"[wizard-validation] user={m.from_user.id} template={template_id} missing={missing} keys={sorted(data.keys())}")
        # Jangan hapus FSM saat validasi gagal. Data yang sudah diisi tetap
        # dipertahankan agar pengguna tidak harus mengulang semuanya.
        await m.answer(
            "❌ Data belum lengkap. Field wajib yang masih kosong: "
            + ", ".join(missing)
            + "\n\nData yang sudah kamu isi tetap tersimpan. "
              "Silakan isi field yang masih kosong."
        )
        # Kembalikan wizard ke field wajib pertama yang kosong.
        first_missing_id=None
        for f in fields:
            if f.get("required") and str(f.get("label") or f.get("id")) in missing:
                first_missing_id=f.get("id")
                break
        if first_missing_id:
            for pos, f in enumerate(fields):
                if f.get("id") == first_missing_id:
                    await state.update_data(index=pos)
                    break
            await ask_next(m,state)
        return
    ...
    try:
        ...
        # A failed build must not poison every future build with a bad DB row.
        try:
            delete_website_by_id(wid)
        except Exception:
            pass
        await progress.edit_text(
            f"⚠️ Website gagal dibuat dan tidak disimpan.\n\n<code>{str(e)[:1000]}</code>",
            parse_mode="HTML"
        )
        await state.clear()
        return
    ...
    try:
        ...
        # The local/published build is valid, so keep the DB row. Admin can
        # retry the same complete build with /admin → Redeploy.
        await progress.edit_text(
            f"⚠️ Website sudah berhasil dibuat, tetapi deploy gagal.\n\n"
            f"🆔 <code>{wid}</code>\n"
            f"Silakan coba <b>Redeploy</b>.\n\n<code>{str(e)[:1000]}</code>",
            parse_mode="HTML"
        )
        await state.clear()
        return
