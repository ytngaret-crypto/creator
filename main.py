...
async def restore_wizard(state, user_id):
    """Restore a durable wizard session only when the in-memory FSM is gone.

    IMPORTANT: while the wizard is active, FSM contains the newest answers.
    Never overwrite that data with an older SQLite snapshot.
    """
    current_state = await state.get_state()

    # Active wizard: keep the live FSM data. SQLite is only a recovery source.
