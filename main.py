...
async def restore_wizard(state, user_id):
    """Restore only when the in-memory FSM is actually gone.

    During an active wizard the FSM contains the newest answer. Restoring the
    older SQLite snapshot at that point would overwrite the answer that was
    just entered (title/intro/gallery/etc.). SQLite is only a recovery source
    when MemoryStorage has lost the FSM state.
    """
    current_state = await state.get_state()
    if current_state == Form.collecting.state:
        return False
