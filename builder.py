...
def _copy_media(manifest, data, target):
    ...
    # Copy file berdasarkan field spec manifest tanpa mengosongkan nilai yang ada
    for fid, typ in _field_specs(manifest):
        val = data.get(fid)
        if not val:
            continue
        if typ in PHOTO_TYPES:
            ...
            data[fid] = _copy_one_media(val, media_dir, many=many)
        elif typ in AUDIO_TYPES:
            data[fid] = _copy_one_media(val, media_dir, many=False)
