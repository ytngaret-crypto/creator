...
def render_one(site, out_root):
    ...
    if not template:
        # A template may have been deleted after a website was created. Keep
        # the already-published copy instead of making every future build fail.
        existing = PUBLISHED / "w" / str(site["id"])
        target = out_root / "w" / str(site["id"])
        if (existing / "index.html").is_file():
            shutil.copytree(existing, target, dirs_exist_ok=True)
            return
        raise RuntimeError(
            f"Template '{site['category']}/{site['template_id']}' tidak ditemukan dan website lama tidak tersedia."
        )
