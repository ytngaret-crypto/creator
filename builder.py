...
def _normalise_data(src: Path, raw_data):
    ...
    # Be tolerant of older records that stored the actual fields under "data".
    if isinstance(data.get("data"), dict):
        nested = dict(data.get("data") or {})
        nested.update({k: v for k, v in data.items() if k != "data"})
        data = nested

    # Normalize keys exactly as manifest IDs (strip accidental whitespace).
    cleaned = {}
    ...
    # Recover values saved by older bot versions under a field label/name
    # instead of the manifest ID. This makes old website records render too.
    lower_keys = {str(k).strip().lower(): k for k in data.keys()}
    for raw in manifest.get("fields", []) if isinstance(manifest.get("fields", []), list) else []:
        if not isinstance(raw, dict):
            continue
        fid = str(raw.get("id") or raw.get("name") or "").strip()
        if not fid or data.get(fid) not in (None, "", []):
            continue
        candidates = [str(raw.get("label") or "").strip(), str(raw.get("name") or "").strip()]
        for candidate in candidates:
            old_key = lower_keys.get(candidate.lower()) if candidate else None
            if old_key is not None and data.get(old_key) not in (None, "", []):
                data[fid] = data[old_key]
                break

    # Older versions sometimes stored a list of intro texts. Expand it into
    # intro1, intro2, ... when those IDs are present in the current manifest.
    intro_list = data.get("introTexts") or data.get("intro_texts")
    if isinstance(intro_list, list):
        for n, value in enumerate(intro_list, 1):
            key = f"intro{n}"
            if key in {fid for fid, _ in _field_specs(manifest)} and data.get(key) in (None, "", []):
                data[key] = value
...
def _copy_one_media(value, media_dir, many=False):
    ...
    # Already-rendered relative media URL: do not try to copy it again.
...
def _copy_media(manifest, data, target):
    ...
    if "gallery" in data:
        data["gallery"] = (
            [str(x).replace("\\", "/") for x in data["gallery"]]
            if isinstance(data["gallery"], list) else data["gallery"]
        )
        if isinstance(data["gallery"], str) and not data["gallery"].startswith("media/"):
            data["gallery"] = _copy_one_media(data["gallery"], media_dir, many=True)
    if "music" in data:
        music = str(data["music"]).replace("\\", "/") if data["music"] else ""
        data["music"] = music if music.startswith("media/") else _copy_one_media(music, media_dir, many=False)
...
def render_one(site, out_root):
    ...
    # A published page must never expose template tokens to visitors.
