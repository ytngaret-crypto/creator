...
async def choose_template(c,state):
    ...
    # Manifest is the single source of truth. Re-read it from the selected
    # template folder instead of trusting cached/derived field metadata.
    manifest_path = Path(t["path"]) / "manifest.json"
    if not manifest_path.is_file():
        await c.answer("Manifest template tidak ditemukan.",show_alert=True)
        return
    try:
        ...
        for raw in raw_fields if isinstance(raw_fields, list) else []:
            ...
            f = dict(raw)
            f["id"] = fid
            f["label"] = str(f.get("label") or fid)
            f["type"] = str(f.get("type") or "text").lower().strip()
            f["required"] = bool(f.get("required", False))
            f["description"] = str(f.get("description") or "").strip()
            fields.append(f)
        if manifest.get("fields") and not fields:
            await c.answer("manifest.json tidak memiliki field yang valid.",show_alert=True)
            return
    except Exception as e:
        await c.answer("manifest.json tidak valid.",show_alert=True)
        await c.message.answer(f"❌ Manifest template rusak: <code>{str(e)[:500]}</code>", parse_mode="HTML")
        return

    await state.update_data(template_id=t["id"],fields=fields,index=0,data={},template_path=t["path"])
    await track_message(state,c.message.message_id)
