...
def _normalise_data(src: Path, raw_data):
    """Load manifest and preserve the submitted field IDs without copying media yet."""
    manifest = _read_manifest(src)
    return manifest, dict(raw_data or {})
...
def _validate_required_data(manifest, data, site_id):
    missing = []
    for fid, typ in _field_specs(manifest):
        # Recover the required flag from the raw manifest because _field_specs
        # intentionally returns only id/type.
        fields = manifest.get("fields", [])
        if isinstance(fields, dict):
            raw = fields.get(fid, {})
            required = bool(raw.get("required", False)) if isinstance(raw, dict) else False
        else:
            raw = next((x for x in fields if isinstance(x, dict) and str(x.get("id") or x.get("name") or "").strip() == fid), {})
            required = bool(raw.get("required", False)) if isinstance(raw, dict) else False
        if not required:
            continue
        value = data.get(fid)
    ...
    if missing:
        raise RuntimeError(
            f"Website {site_id}: field wajib belum lengkap: {', '.join(missing)}"
        )


def _copy_media(manifest, data, target):
    """Copy each source media file exactly once and keep canonical aliases in sync.

    Important: after a field is converted to a relative ``media/...`` path, it
    must not be passed through the copier a second time. The old implementation
    did exactly that for fields named ``gallery``/``music``, which erased valid
    photo/audio paths.
    """
    ...
    # Legacy templates may use canonical placeholders without declaring a
    # corresponding manifest field. Only copy these when they were NOT already
    # processed as real manifest fields.
    if "gallery" in data and "gallery" not in processed:
        data["gallery"] = _copy_one_media(data.get("gallery"), media_dir, many=True)
    if "music" in data and "music" not in processed:
        data["music"] = _copy_one_media(data.get("music"), media_dir, many=False)

    # For templates with arbitrary photo/audio IDs, expose the first matching
    # field through legacy aliases when the aliases are not already present.
    photo_ids = [fid for fid, typ in specs if typ in PHOTO_TYPES]
    audio_ids = [fid for fid, typ in specs if typ in AUDIO_TYPES]
...
def render_one(site, out_root):
    ...
    _validate_required_data(manifest, data, site["id"])
...
def build_all():
    """Build all sites in a staging directory and replace the live tree only on success."""
    import tempfile

    parent = PUBLISHED.parent
    parent.mkdir(parents=True, exist_ok=True)
    staging = Path(tempfile.mkdtemp(prefix=f".{PUBLISHED.name}_staging_", dir=parent))
    try:
        for site in get_all_websites():
            render_one(site, staging)
        # Redirects are part of the successful build, never the old live tree.
        write_netlify_redirects(staging)

        old = None
        if PUBLISHED.exists():
            old = parent / f".{PUBLISHED.name}_old"
            if old.exists():
                shutil.rmtree(old)
