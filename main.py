...
async def done(m,state):
    ...
    # Jika field terakhir (mis. musik) sudah diterima, proses pembuatan
    # sudah/akan dilanjutkan oleh handler field tersebut. Jangan akses
    # fields[i] karena i bisa sama dengan len(fields).
    if i >= len(fields):
        await finish_creation(m,state)
        return
    ...
    if f.get("type")=="photos":
        ...
        await m.answer("ℹ️ /selesai digunakan untuk menyelesaikan upload foto. Untuk musik, cukup kirim audio/voice atau gunakan /skip.")
