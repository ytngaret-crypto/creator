...
async def restore_wizard(state, user_id):
    """Restore a wizard only when the in-memory FSM has actually been lost.

    IMPORTANT: Never overwrite an active FSM with an older SQLite snapshot.
    Every answer is first written to FSM and then persisted; restoring on every
    message would roll the wizard back to the previous answer and can make
    required fields appear missing at build time.
    """
    current = await state.get_state()
    if current:
        return True
