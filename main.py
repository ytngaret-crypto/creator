...
async def persist_wizard(state, user_id):
    """Persist the active creation wizard outside aiogram MemoryStorage.

    Railway/redeploys or multiple bot workers must not erase title/intros/photos
    already collected. The SQLite session is the durable source for the wizard;
    FSM remains only the fast in-process cache.
    """
    try:
        ...
        # Persistence failure must be visible in logs but should not crash a
        # Telegram update halfway through the wizard.
        import logging
...
async def ensure_wizard_loaded(state, user_id):
    """Load SQLite only when the live FSM is not usable.

    SQLite is recovery storage, not a second source that may overwrite
    newer answers already present in the active FSM.
    """
    current = await state.get_data()
    active = await state.get_state()
    if active == Form.collecting.state and current.get("template_id") and current.get("fields"):
        return current

    if await restore_wizard(state, user_id):
        return await state.get_data()

    return current

def _merge_wizard_data(current, saved):
    """Prefer live FSM answers over an older durable snapshot."""
    ...
    saved_data = dict(saved.get("data") or {})
    live_data = dict(current.get("data") or {})
    saved_data.update(live_data)
    merged["data"] = saved_data
    return merged
...
async def skip_current(m,state):
    await ensure_wizard_loaded(state,m.from_user.id)
    d=await state.get_data()
    fields=d.get("fields",[])
    i=d.get("index",0)
    if i >= len(fields):
...
async def finish_creation(m,state):
    # The active FSM is authoritative. SQLite is only a recovery source.
    live = await state.get_data()
    saved = get_wizard_session(m.from_user.id) or {}
    d = _merge_wizard_data(live, saved)
    ...
    try:
        ...
        for f in fields:
            ...
            if typ == "text_list":
                ...
                if not value:
                    missing.append(fid)
            elif typ in {"photos","image","images","gallery"}:
        ...
        await m.answer(
            f"❌ Website belum dibuat.\n\n<code>{str(e)[:1000]}</code>",
            parse_mode="HTML"
        )
        return

    # Persist exactly the validated payload before building.
    save_wizard_session(m.from_user.id, {**d, "data": data})

    wid=create_website(m.from_user.id,category,template_id,data)
    progress=await m.answer(
        "⏳ <b>Data lengkap.</b> Website sedang dibuat dan dipublikasikan...",
        parse_mode="HTML"
    )
    try:
        published=build_all()
